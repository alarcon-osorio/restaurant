...
public interface RepositoryCloseDay extends JpaRepository<CloseDay, String> {
    ...
    @Query(value = "SELECT id, name, cant, table_number, CAST(time AS DATE) as time,  price * cant as total_item, payment_method FROM restaurant.service_order where CAST(time AS DATE) = ?1 AND close = 1;", nativeQuery = true)
    ...
    @Query(value = "SELECT sum(cant * price) as total FROM restaurant.service_order where  CAST(time AS DATE) = ?1 AND close = 1;", nativeQuery = true)
